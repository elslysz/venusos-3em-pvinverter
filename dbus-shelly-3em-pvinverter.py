...
class DbusShelly1pmService:
  def __init__(self, servicename, paths, productname='Shelly 3EM-as-1PM', connection='Shelly 3EM-as-1PM HTTP JSON service'):
  ...
  def _getShellyData(self):
    ...
    # check for response
    if not meter_r:
        raise ConnectionError("No response from Shelly 3EM-1PM - %s" % (URL))
  ...
  def _update(self):
    try:
       ...
       phase_3em = int(config['ONPREMISE']['3EMSelectedPhase'])-1
       ...
       for phase in ['L1']:
         ...
         if phase == pvinverter_phase:
           power = meter_data['emeters'][phase_3em]['power']
           total = meter_data['emeters'][phase_3em]['total']
           voltage = meter_data['emeters'][phase_3em]['voltage']
           current = meter_data['emeters'][phase_3em]['current']
